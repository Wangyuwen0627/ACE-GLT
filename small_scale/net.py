...
gcn_mask = fn.src_mul_edge('h', 'mask', 'm')
gcn_msg = fn.copy_src(src="h", out="m")
gcn_reduce = fn.sum(msg="m", out="h")

class ApplyNodeFunc(nn.Module):
    """
        This class is used in class net_gin
...
class NodeApplyModule(nn.Module):
    """
        This class is used in class net_gcn
        Update the node feature hv with MLP
    """
...
class GCNLayer(nn.Module):
    ...
    def forward(self, g, feature):
        g.ndata["h"] = feature
        g.update_all(gcn_mask, gcn_reduce)
        g.apply_nodes(func=self.apply_mod)
        return g.ndata.pop("h")

class GCNLayer_baseline(nn.Module):
    def __init__(self, in_feats, out_feats, activation):
        super(GCNLayer_baseline, self).__init__()
        self.apply_mod = NodeApplyModule(in_feats, out_feats, activation)
    def forward(self, g, feature):
        g.ndata["h"] = feature
...
class net_gcn(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, graph):
        super(net_gcn, self).__init__()
        self.gcn1 = GCNLayer(input_dim, hidden_dim, F.relu)  # 第一层GCN
        self.gcn2 = GCNLayer(hidden_dim, output_dim, None)
...
class net_gcn_baseline(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, graph):
        super(net_gcn_baseline, self).__init__()
        self.gcn1 = GCNLayer_baseline(input_dim, hidden_dim, F.relu)  # 第一层GCN
        self.gcn2 = (GCNLayer_baseline(hidden_dim, output_dim, None))
        self.edge_num = graph.all_edges()[0].numel()
