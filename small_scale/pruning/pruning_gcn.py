...
def add_mask(model, init_mask_dict=None):
    if init_mask_dict is None:
        mask1_train = nn.Parameter(torch.ones_like(model.gcn1.apply_mod.linear.weight))
        mask1_fixed = nn.Parameter(torch.ones_like(model.gcn1.apply_mod.linear.weight), requires_grad=False)
        mask2_train = nn.Parameter(torch.ones_like(model.gcn2.apply_mod.linear.weight))
        mask2_fixed = nn.Parameter(torch.ones_like(model.gcn2.apply_mod.linear.weight), requires_grad=False)
    else:
    ...
    AddTrainableMask.apply(model.gcn1.apply_mod.linear, 'weight', mask1_train, mask1_fixed)
    AddTrainableMask.apply(model.gcn2.apply_mod.linear, 'weight', mask2_train, mask2_fixed)

def generate_mask(model):
    mask_dict = {}
    mask_dict['mask1'] = torch.zeros_like(model.gcn1.apply_mod.linear.weight)
    mask_dict['mask2'] = torch.zeros_like(model.gcn2.apply_mod.linear.weight)
    return mask_dict

def subgradient_update_mask(model, args):
    model.adj_mask1_train.grad.data.add_(args['s1'] * torch.sign(model.adj_mask1_train.data))
    model.gcn1.apply_mod.linear.weight_mask_train.grad.data.add_(
        args['s2'] * torch.sign(model.gcn1.apply_mod.linear.weight_mask_train.data))
    model.gcn2.apply_mod.linear.weight_mask_train.grad.data.add_(
        args['s2'] * torch.sign(model.gcn2.apply_mod.linear.weight_mask_train.data))
...
def get_final_mask_epoch(model, adj_percent, wei_percent):
    ...
    weight_dict['model_weight1'] = model.gcn1.apply_mod.linear.state_dict()['weight_mask_train']
    weight_dict['model_weight2'] = model.gcn2.apply_mod.linear.state_dict()['weight_mask_train']
...
def print_sparsity(model):
    ...
    weight1_total = model.gcn1.apply_mod.linear.weight_mask_fixed.numel()
    weight2_total = model.gcn2.apply_mod.linear.weight_mask_fixed.numel()
    weight_total = weight1_total + weight2_total
    weight1_nonzero = model.gcn1.apply_mod.linear.weight_mask_fixed.sum().item()
    weight2_nonzero = model.gcn2.apply_mod.linear.weight_mask_fixed.sum().item()
...
def add_trainable_mask_noise(model, c):
    ...
    model.gcn1.apply_mod.linear.weight_mask_train.requires_grad = False
    model.gcn2.apply_mod.linear.weight_mask_train.requires_grad = False
    ...
    rand2 = (2 * torch.rand(model.gcn1.apply_mod.linear.weight_mask_train.shape) - 1) * c
    rand2 = rand2.to(model.gcn1.apply_mod.linear.weight_mask_train.device)
    rand2 = rand2 * model.gcn1.apply_mod.linear.weight_mask_train
    model.gcn1.apply_mod.linear.weight_mask_train.add_(rand2)
    rand3 = (2 * torch.rand(model.gcn2.apply_mod.linear.weight_mask_train.shape) - 1) * c
    rand3 = rand3.to(model.gcn2.apply_mod.linear.weight_mask_train.device)
    rand3 = rand3 * model.gcn2.apply_mod.linear.weight_mask_train
    model.gcn2.apply_mod.linear.weight_mask_train.add_(rand3)
    model.adj_mask1_train.requires_grad = True
    model.gcn1.apply_mod.linear.weight_mask_train.requires_grad = True
    model.gcn2.apply_mod.linear.weight_mask_train.requires_grad = True
...
def soft_mask_init(model, init_type, seed):
    ...
    if init_type == 'all_one':
        ...
        model.adj_mask1_train.requires_grad = True
        init.kaiming_uniform_(model.gcn1.apply_mod.linear.weight_mask_train, a=math.sqrt(5))
        model.gcn1.apply_mod.linear.weight_mask_train.requires_grad = False
        model.gcn1.apply_mod.linear.weight_mask_train.mul_(model.gcn1.apply_mod.linear.weight_mask_fixed)
        model.gcn1.apply_mod.linear.weight_mask_train.requires_grad = True
        init.kaiming_uniform_(model.gcn2.apply_mod.linear.weight_mask_train, a=math.sqrt(5))
        model.gcn2.apply_mod.linear.weight_mask_train.requires_grad = False
        model.gcn2.apply_mod.linear.weight_mask_train.mul_(model.gcn2.apply_mod.linear.weight_mask_fixed)
        model.gcn2.apply_mod.linear.weight_mask_train.requires_grad = True
        ...
        model.adj_mask1_train.mul_(model.adj_mask2_fixed)
        model.adj_mask1_train.requires_grad = True
        init.normal_(model.gcn1.apply_mod.linear.weight_mask_train, mean=mean, std=std)
        model.gcn1.apply_mod.linear.weight_mask_train.requires_grad = False
        model.gcn1.apply_mod.linear.weight_mask_train.mul_(model.gcn1.apply_mod.linear.weight_mask_fixed)
        model.gcn1.apply_mod.linear.weight_mask_train.requires_grad = True
        init.normal_(model.gcn2.apply_mod.linear.weight_mask_train, mean=mean, std=std)
        model.gcn2.apply_mod.linear.weight_mask_train.requires_grad = False
        model.gcn2.apply_mod.linear.weight_mask_train.mul_(model.gcn2.apply_mod.linear.weight_mask_fixed)
        model.gcn2.apply_mod.linear.weight_mask_train.requires_grad = True
        ...
        model.adj_mask1_train.mul_(model.adj_mask2_fixed)
        model.adj_mask1_train.requires_grad = True
        init.uniform_(model.gcn1.apply_mod.linear.weight_mask_train, a=a, b=b)
        model.gcn1.apply_mod.linear.weight_mask_train.requires_grad = False
        model.gcn1.apply_mod.linear.weight_mask_train.mul_(model.gcn1.apply_mod.linear.weight_mask_fixed)
        model.gcn1.apply_mod.linear.weight_mask_train.requires_grad = True
        init.uniform_(model.gcn2.apply_mod.linear.weight_mask_train, a=a, b=b)
        model.gcn2.apply_mod.linear.weight_mask_train.requires_grad = False
        model.gcn2.apply_mod.linear.weight_mask_train.mul_(model.gcn2.apply_mod.linear.weight_mask_fixed)
        model.gcn2.apply_mod.linear.weight_mask_train.requires_grad = True
    else:
