import __init__
import torch
from sparse.torch_vertex import GENConv
from sparse.torch_nn import norm_layer
...
class DeeperGCN(torch.nn.Module):
    def __init__(self, args):
        ...
        self.num_original_edge = 79122504
    ...
    def forward(self, x, node_index, edge_index, edge_attr, edge_mask=None):
        '''
        x: subgraph node feature
        node_index: real node index
        edge_index: subgraph adj_matrix edge index
        edge_attr : subgraph edge attr
        '''
        
        if edge_mask is not None:
            self.edge_mask = nn.Parameter(edge_mask, requires_grad=True)
        ...
        if self.block == 'res+':
            ...
            h = self.gcns[0](h, edge_index, edge_emb, self.edge_mask)
            ...
            if self.checkpoint_grad:
                for layer in range(1, self.num_layers):
                    ...
                    if layer % self.ckp_k != 0:
                        res = checkpoint(self.gcns[layer], h2, edge_index, edge_emb, self.edge_mask)
                        h = res + h
                    else:
                        h = self.gcns[layer](h2, edge_index, edge_emb, self.edge_mask) + h
