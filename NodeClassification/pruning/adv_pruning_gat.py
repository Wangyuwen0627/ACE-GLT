    ...
    for layer in range(2):
        for head in range(8):
            ...
            mask_1 = torch.where(model.layers[layer].heads[head].attn_fc.state_dict()['weight_mask_train'] != 0, ones_1,
                                 zeros_1)
            temp1_1 = torch.where(best_epoch_mask_adv[key_train1] == 1, ones_1, mask_1)
            temp2_1 = torch.where(best_epoch_mask_origin[key_train1] == 1, zeros_1, temp1_1)
...
def get_final_mask_round_adv(model, best_epoch_mask_adv, best_epoch_mask_origin):
    ...
    for layer in range(2):
        for head in range(8):
            ...
            mask_1 = torch.where(model.layers[layer].heads[head].attn_fc.state_dict()['weight_mask_train'] != 0, ones_1,
                                 zeros_1)
            temp1_1 = torch.where(best_epoch_mask_adv[key_train1] == 1, zeros_1, mask_1)
            temp2_1 = torch.where(best_epoch_mask_origin[key_train1] == 1, ones_1, temp1_1)
            mask_dict[key_train1] = temp2_0
            mask_dict[key_train2] = temp2_1
            if layer == 1: break
    return mask_dict
