...
def get_final_mask_epoch_adv(model, wei_thre_index):
    ...
    wei_thre = wei_y[wei_thre_index]
    mask_dict = {}
    mask_dict['weight1_mask'] = get_each_mask_adv(model.net_layer[0].state_dict()['weight_mask_train'], wei_thre)
    mask_dict['weight2_mask'] = get_each_mask_adv(model.net_layer[1].state_dict()['weight_mask_train'], wei_thre)
...
def get_final_mask_round_origin(model, best_epoch_mask_adv, best_epoch_mask_origin):
    ones_0 = torch.ones_like(model.net_layer[0].weight_mask_train)
    zeros_0 = torch.zeros_like(model.net_layer[0].weight_mask_train)
    mask_0 = torch.where(model.net_layer[0].weight_mask_train != 0, ones_0, zeros_0)
    ...
    ones_1 = torch.ones_like(model.net_layer[1].weight_mask_train)
    zeros_1 = torch.zeros_like(model.net_layer[1].weight_mask_train)
    mask_1 = torch.where(model.net_layer[1].weight_mask_train != 0, ones_1, zeros_1)
