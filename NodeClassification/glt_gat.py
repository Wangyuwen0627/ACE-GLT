    ...
    for round in range(rounds):
        ...
        if round != 0:
            ...
            if similarity_origin_adv > 0.6:
                ...
                wei_thre_index, best_epoch_mask_adv = adv_pruning_gat.get_final_mask_epoch(last_best_adv_model_copy,
                                                                                           recover_rate)
                best_epoch_mask_origin = adv_pruning_gat.get_final_mask_epoch_adv(last_best_origin_model_copy,
                                                                                  wei_thre_index)
                best_mask_refine_origin = adv_pruning_gat.get_final_mask_round_origin(last_best_adv_model_copy,
                                                                                      best_epoch_mask_adv,
                                                                                      best_epoch_mask_origin)
                best_mask_refine_adv = adv_pruning_gat.get_final_mask_round_adv(last_best_adv_model_copy,
                                                                                best_epoch_mask_adv,
                                                                                best_epoch_mask_origin)
            rounds = rounds + 1
            continue
